...
impl<'d, T: Instance> I2cSlave<'d, T> {
    ...
    /// If not, enable interrupts for waiting
                ...
                match state {
                    ...
                    TransactionState::Receiving => {
                        ...
                        if star1.addr() {
                            ...
                            writeln!(uart, "BUG: addr seen while rx\r").unwrap();
                            return Poll::Ready(Ok(Command::Write(rx_len)));
                        }
            ...
            loop {
                ...
                if star1.tx_e() {
                    ...
                    if tx_pos < buf.len() {
                        ...
                        i2c.datar().write(|r| r.set_datar(buf[tx_pos]));
                        tx_pos += 1;
    ...
    pub async fn respond_till_stop(
        &mut self,
        fill: u8,
        uart: &mut impl Write,
    ) -> Result<(), Error> {
        let buf = [fill; 4];
        loop {
            match self.respond_to_read(&buf, uart).await {
    ...
    pub async fn respond_and_fill(
        &mut self,
        buf: &[u8],
        fill: u8,
        uart: &mut impl Write,
    ) -> Result<ReadStatus, Error> {
        let result = self.respond_to_read(buf, uart).await?;
        if result == ReadStatus::NeedMoreBytes {
            self.respond_till_stop(fill, uart).await?;
            Ok(ReadStatus::Done)
        } else {
            Ok(result)
        }
